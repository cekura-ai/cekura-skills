...
2. **`/report-bug` command** — Users (or Claude, prompted by the hook) can run this to:
   ...
   - File a GitHub issue on `cekura-ai/claude-skills` via `gh issue create`
...
- It lands in the `cekura-ai/claude-skills` repo with the `bug` label
...
- The marketplace is registered by URL (`https://github.com/cekura-ai/claude-skills.git`) — users get updates by pulling
