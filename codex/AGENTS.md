...
17. **Changing the project call cap for a test, or capping every scenario** — Unset `max_duration` inherits the project owner's `max_call_duration`, which is normally right. Give a scenario its own `max_duration` (10–3600 s) only when it must end within a known time (idle/timeout/hold tests, a user-requested limit, a short flow on a paid transport where a stalled agent would burn the whole cap).
