...
- Poll every ~10s, report progress every ~30s; if 0/N after ~2 min, stop, tell the user, retry once with a batch ≤5 — never loop "keep waiting" indefinitely. Report elapsed time and counts from the actual progress data — never estimate or inflate elapsed time.
