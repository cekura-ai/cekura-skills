...
**Creating a behavioral (`instruction`) scenario is not what this command is for** — those are always generated via `/autogen-eval` → `scenarios_generate_bg`, even a single one. Like behavioural scenarios, conditional-action scenarios are **generated by default** — `/autogen-eval` with `simulation_type: "conditional_actions"`, with every tag requirement (DTMF entry, hold length, IVR menu, voicemail) written into `extra_instructions`. This command's create path is for what generation cannot serve: the user dictates the exact turn-by-turn script, or a value must be exact to the decimal (an infra test bracketing a timeout threshold). See the Step 2 gate below. Updating any existing scenario, of either type, belongs here.
...
- **Deterministic (conditional actions) → generate first.** Run `scenarios_generate_bg` with `simulation_type: "conditional_actions"`, putting the flow and every tag requirement (DTMF entry, hold length, IVR menu, voicemail, interruption) in `extra_instructions`; then walk the result through the fields below and patch what is off. Stay here for a direct create only when the user dictates the exact turn-by-turn script, or when a timing value must be exact (an infra test bracketing a timeout threshold).
