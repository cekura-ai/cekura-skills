...
**Note:** Language-specific personalities may not be enabled on all projects. Only if the language-matched personality returns a "Personality is not enabled" error, fall back to a normal English personality (from `personalities_list language=en`) and try to set `scenario_language` on the create call (the MCP create/update tools may not expose the field — if it doesn't stick, disclose it and point the user to the dashboard; never claim it was set when it wasn't) — never default to an English personality for a non-English scenario without trying the language-matched personality first.
