...
**Note:** Language-specific personalities may not be enabled on all projects. Only if the language-matched personality returns a "Personality is not enabled" error, remember `scenario_language` is **coupled to the personality's language by design** (the API rejects a mismatch — don't work around it): enable the predefined one for the project, or create/fork a Normal personality in the target language and use it (multilingual voice models handle any supported language; mention the new personality in your summary). Only if that write actually fails (permissions, plan limits) or the user declines, fall back to a normal English personality with `scenario_language` staying `en`, keep the caller-facing content in the target language, and disclose the TTS limitation plainly — never default to an English personality for a non-English scenario without trying the language-matched path first.
