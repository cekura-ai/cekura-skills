...
**Creating a behavioral (`instruction`) scenario is not what this command is for** — those are always generated via `/autogen-eval` → `scenarios_generate_bg`, even a single one. Conditional-action scenarios are **also generated by default** — `/autogen-eval` with `simulation_type: "conditional_actions"`, with every tag requirement (DTMF entry, hold length, IVR menu, voicemail) written into `extra_instructions`. This command's create path is for what generation cannot serve: the user dictates the exact turn-by-turn script, or a value must be exact to the decimal (an infra test bracketing a timeout threshold). See the Step 2 gate below. Updating any existing scenario, of either type, belongs here.
