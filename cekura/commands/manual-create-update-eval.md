...
`skill_name="manual-create-update-eval"`, `verification_tag="ack:manual-create-update-eval:5m4p7c"`, and `plugin_version="0.10.9"`. If a conversation/session ID is available (e.g. you
...
Use `mcp__cekura__personalities_list` to list available personalities, filtered by the chosen language if possible. **Pass only `language=<code>` — never `project_id`:** predefined personalities are global (no project owner) and a `project_id` filter silently excludes all of them.
...
**Note:** Language-specific personalities may not be enabled on all projects. Only if the language-matched personality returns a "Personality is not enabled" error, fall back to a normal English personality (from `personalities_list language=en`) via a two-step: create **without** `scenario_language` (the API rejects a create where it differs from the personality's language), then PATCH `scenario_language` by itself (if the update tool doesn't expose the field, disclose it and point the user to the dashboard; never claim it was set when it wasn't) — never default to an English personality for a non-English scenario without trying the language-matched personality first.
