...
> In **Claude Code CLI**, a `SessionStart` hook auto-runs the re-pin commands below on every launch, so most CLI users are already current. (Claude Desktop uses a separate plugin store with its own update setting — the hook doesn't reach it; update there from the Desktop plugin panel.) Run this command to force an upgrade now (mid-session), or to handle the stale-layout cohort the hook can't fix.
