...
| `personalities` | Always pick the "Normal" personality matched to the agent's language via `personalities_list language=<code>` (English included: `language=en`; multilingual `language=multi` if several languages are involved; `language` is the only filter needed) |
...
Poll `mcp__cekura__results_retrieve(id=result_id)` until `status` is `completed`, `failed`, `timeout` or `cancelled`. The result stays in progress until every run has ended; `completed` then means at least one run completed, not that runs passed — count passes from `success_runs_count` against `total_runs_count`, since `failed_runs_count` leaves out runs that errored or timed out. Use `mcp__cekura__results_list` to monitor overall progress. If a run hangs, use `mcp__cekura__end_call` to terminate it.
