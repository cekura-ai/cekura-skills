...
Reproduce at **minimum cost**: first decide whether the failure can be made deterministic — force the trigger via scenario construction or, for a local bot, temporary fault injection in code (simulated LLM/provider failure, dropped tool result; mark lines `# CEKURA-REPRO-INJECT`, keep active through verification, never in the fix diff). Deterministic → **one simulation run, must fail**; a pass means the trigger isn't forced — fix the forcing, don't add runs. Only LLM prompt/workflow behavior that can't be forced goes stochastic: run the smallest batch expected to fail at least twice (`⌈2/p̂⌉` from the observed failure rate, min 4, max 10) and require **≥2 failing runs**.
