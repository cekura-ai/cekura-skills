...
`self_hosted` is the umbrella mode for any agent whose live runtime is **not** a managed provider that this skill can PATCH directly. The user owns the live agent code; this skill operates on the user's source file as the source-of-truth artifact.

Currently the only supported self-hosted flavor is **websocket** — a custom websocket server (e.g., `wss://...ngrok.io/...` pointing at a Python/Node/Go process) that exposes the system prompt as a string constant in source code. Tool definitions usually live in the same file. When no live websocket is reachable, the mode degrades to a pasted-prompt / pasted-failures variant.

See [`websocket.md`](websocket.md) for the full procedure: gate, prompt-file discovery, code-edit apply path via `Edit`, restart-server gate, the pasted-failure degenerate variant, and edge cases.

## Routing (Phase 1.2)

When `assistant_provider` resolves to `self_hosted` (either directly or via the unsupported-tag fallback for `custom` / `agentforce` / similar), route straight to the websocket flavor.

If the user can point at the source file the agent reads its system prompt from, use the **`file`** variant (skill edits the source directly; user restarts the server before re-validation). If there is no file reachable from this workspace (server is on a different machine, the user is iterating in a notebook, the user doesn't want the skill touching the file), use the **`offline`** variant (pasted prompt text, pasted failures, no `Edit`, no auto re-validation). See `websocket.md` § "Degenerate variant: no live websocket".
...
- **Validation runs through Cekura** when a live agent is reachable. Cekura drives the scenario, opens a websocket to the user's URL, captures the transcript, and runs metrics. The skill consumes those results the same way as VAPI mode.
- **Restart is automated when `redeploy_command` is configured** — the skill runs it after each apply step (Phase 4.1) so the live agent is ready before validation. When the command isn't configured, the loop falls back to either the auto-mode "trust and surface no-change after the fact" behavior or the non-auto manual restart gate. See "Redeploy command flow" below.

## Redeploy command flow

The single biggest source of friction in the legacy self-hosted loop is the per-iteration "restart your server" pause. With `redeploy_command` configured, the loop runs end-to-end autonomously: apply → restart → validate → diagnose → apply → ... Without it, the user has to manually unblock every iteration.
...
```
For end-to-end automation, I can run your restart automatically after each
...
- The resolved mode is `vapi` (VAPI edits land live; nothing to restart).
...
- Empty string or "skip" → same as `"manual"` — surface to the user that you've recorded the manual fallback rather than treating empty as "no restart needed".
- Anything else → treat as a shell command. Do not validate the command's correctness in Phase 1; the user owns that.

### Execution (Phase 4.1)

After the apply step lands (file edits) and before Step 4.2 sync verification:

1. Run the recorded `redeploy_command` via the Bash tool. Use a reasonable timeout (default 120s; bump to 600s if the user's command contains hints like `deploy`, `cloud`, `image`, `push`).
...
For SSH-to-remote restarts, the user's SSH config must support non-interactive sessions (keys, no MFA prompts). If the restart command hangs waiting for input, the Bash tool will time out — surface this clearly and ask the user to make the command non-interactive.

### What this skill will NOT do

- **Modify the user's deploy infrastructure.** If the systemd unit is misconfigured or the docker-compose file is broken, that's the user's problem to fix — surface and pause.
- **Verify the new prompt is actually live.** There's no general-purpose way to check this across runtimes. The Step 4.5 no-change detector is the only signal the skill has; if results look unchanged, surface the "restart may not have taken effect" hypothesis.
