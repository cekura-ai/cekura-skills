...
**Two top-level modes.** The skill organizes providers under `providers/`:
...
The `providers/self-hosted/overview.md` file documents the self-hosted umbrella and shared characteristics.
...
- `self_hosted_flavor` (`websocket`) — explicit override for the self-hosted sub-flavor (currently only `websocket` is supported).
...
When in doubt, ask. A short clarifying question costs less than a wrong PATCH against a live agent or a wasted iteration. The "don't pre-emptively pause" rule applies to *per-iteration* user-side gates only — auto mode runs validation directly after each apply without asking "have you restarted?" each time, because the one-time `redeploy_command` collected at Phase 1.4 either handles the restart automatically (real command) or has explicit user buy-in to a manual cadence (`"manual"` sentinel). Do NOT use this rule to skip Phase 1.4 itself, or to skip clarifying which file is the live source — those are one-time setup questions, not per-iteration pauses.

The four phases run in order, with the last looping until the agent passes:

1. **Phase 1 — Verify Agent and Provider Support.** Resolve mode and sub-flavor. For `agent_id` inputs, fetch the agent and gate on `assistant_provider`. Route to one of: `providers/vapi/` or `providers/self-hosted/websocket.md`. Each loads the source-of-truth artifacts the rest of the loop edits against.
...
- **`self_hosted`, `custom`, `agentforce`, or any other non-VAPI tag** → route into the self-hosted / websocket sub-flavor (see [`providers/self-hosted/overview.md`](providers/self-hosted/overview.md) and [`providers/self-hosted/websocket.md`](providers/self-hosted/websocket.md)). If the user doesn't have a live websocket and doesn't want to iterate offline, halt.
...
For the exact VAPI error-message shape, the Retell-specific note, and 404 handling, see [`providers/vapi/phase-1-fetch.md`](providers/vapi/phase-1-fetch.md). For the self-hosted umbrella, see [`providers/self-hosted/overview.md`](providers/self-hosted/overview.md).
...
Skipped for VAPI (edits land live; nothing to redeploy) and for the websocket `offline` variant (no live agent at all). For the **websocket `file` variant**, this step is a **hard gate**: do NOT proceed to Phase 2 until the `redeploy_command` field is resolved to one of three explicit values — a shell command, the literal `"manual"`, or an explicit user-confirmed "no live target to restart" (rare; usually means the run should have been routed to `offline` variant instead). Skipping Phase 1.4 and hoping the user restarts their server between iterations is the single most common reason this skill produces phantom "prompt edits didn't help" iterations — the edits never reached the running process, but the no-change detector at Step 4.5 only catches it after the fact, by which point an iteration of cap is already burned.

Auto mode does NOT exempt this step. `auto_mode: true` skips per-iteration *diff approval* and per-iteration *user-side restart pauses*; it does not skip the one-time setup question that defines HOW the restart happens. Asking once at Phase 1.4 is precisely what enables auto-mode to be autonomous — without it, auto-mode is strictly worse than `auto_mode: false`, because non-auto would at least have paused at each iteration's apply step for a manual restart.
...
**Pre-flight check (fail closed).** Before any Step 2.x work, verify Phase 1 is complete:

- Mode and sub-flavor resolved? (`vapi` / `websocket-file` / `websocket-offline`)
- Source-of-truth artifacts loaded? (VAPI: `/assistant/{id}` + tools; websocket-file: the correct live source file, confirmed via grep when ambiguous)
- **Self-hosted live target**: `redeploy_command` resolved to a shell command or `"manual"`? If not, go back to Phase 1.4 — do NOT begin Phase 2.

If any of the above is unresolved, ask the user the specific clarifying question and wait for an answer. Skipping this pre-flight is the foot-gun documented in Common Pitfalls; the rest of the loop assumes all three are settled.
...
- **End-of-call attribution (Signal 5: who hung up).** For every kept failure, record who terminated the call. The field name varies by provider — check, in order: (a) the run's `metadata.ended_reason` on the Cekura run record (self-hosted runs; common values include `main-agent-ended-call`, `testing-agent-ended-call`, `timeout`, `client-disconnect`); (b) the provider call's `endedReason` field (VAPI: `assistant-said-end-call-phrase`, `assistant-ended-call`, `customer-ended-call`, `silence-timed-out`, etc.); (c) the last few transcript turns — if the final turn is the main agent saying a farewell (e.g., "Thank you for calling..., bye!") and the testing-agent script hadn't completed its required actions yet, the agent ended the call prematurely. Cross-reference with the failure verdict: a failed scenario where `ended_reason == "main-agent-ended-call"` AND the transcript is short relative to the scenario's required steps is a strong signal that the agent ended early on its own. This is qualitatively different from "agent followed the prompt poorly" — the call literally cut off before the scenario could play out, so prompt edits that target later behavior are wasted work.
...
- **Upstream hand-off recommendations** — for failures rooted in missing / wrong dynamic variables, no prompt or tool edit fixes the issue. Surface the variable mismatch with a concrete pointer to where it should be set (test profile, scenario config, squad / project defaults, upstream caller). In the websocket `offline` variant, also surface tool-shaped findings here. In VAPI and websocket `offline`, also surface code-shaped findings here.
...
- **Agent ended the call early.** `ended_reason == "main-agent-ended-call"` (or VAPI `endedReason` ∈ {`assistant-ended-call`, `assistant-said-end-call-phrase`}) on a failed run where the transcript is materially shorter than the scenario's required steps. Specifically: (a) the scenario's expected-outcome bullets describe steps that never appear in the transcript, (b) the agent says a farewell ("thanks, bye", "have a great day") before those steps complete, and (c) the testing agent never sends its terminal scenario-end message. Common root causes to investigate: the agent's "End Conversation" prompt rules are too permissive (e.g., the agent treats "okay" or "thanks" as a closure signal); the orchestration code's end-of-call detection logic fires on any agent message containing certain keywords; or the system prompt lacks an explicit gate that all required fields must be collected before ending. Distinguish from the surface symptom — "agent skipped a field" looks identical from the verdict alone, but the fix surface is very different (prompt-rule edit vs. checklist edit). If this is the diagnosis, propose either: a prompt edit to "End Conversation" / "Closure" rules requiring all qualification steps to be complete before farewell; OR an orchestration-code edit that prevents the websocket loop from accepting a closing turn before required state is captured.
...
- **`redeploy_command == "manual"` (or unset and `auto_mode: false`)** → fire the websocket restart gate. Wait for explicit user confirmation (`done` / `restarted` / `redeployed` / `yes`).
...
6. **Same failure shape on the same scenario for three consecutive iterations** → stop iterating at the same edit surface. Two no-change iterations already proved the prompt/tool layer can't fix this issue under the current model + flow; a third same-shape failure means iter 4 of the same kind of edit is wasted compute. Surface the no-change pattern explicitly (which scenario, which sub-outcome bullet, what was tried each iter), present architectural alternatives, and **wait for the user to pick one** before doing anything else. Alternatives to offer:
   - **Switch to a stronger model** — single-line change (e.g. `gpt-4o-mini → gpt-4o` in the user's source file, or the equivalent VAPI assistant `model.model` field). Most reliable fix for "model lacks instruction-following on long checklists" failures; ~10× per-token cost.
   - **Programmatic guard in orchestration code** (websocket / `file` only) — e.g. before allowing a recommendation turn, scan the transcript for the missing field's question pattern and inject a forced system nudge if absent. Invasive but deterministic.
   - **Restructure the agent flow** — replace natural-conversation prompting with explicit named states (qualification → recommendation gated on collected fields). Larger refactor; warranted when many scenarios share the same failure mode.
   - **Hand off to `cekura-eval-design` / `cekura-metric-improvement`** if the evaluator's grading is the issue (e.g. agent collected equivalent information in a different phrasing the evaluator didn't recognize). Don't propose this casually — only when the failed bullet's text suggests the evaluator may be the problem.
   Do NOT autonomously pick one of these even in auto mode — each carries real cost. The exception is when the user has already explicitly directed an option in the conversation (e.g. "switch to gpt-4o").

7. **Kept = 0 but total > 0** (failures exist but the voice/channel filter discarded them all) → do **not** exit yet. Work in order: (1) **re-classify with fresh eyes** — a tool error *handled badly* by the agent is a prompt issue; for squads, re-attribute by speaker. (2) **Consider mitigation edits** — both prompt (better retries / fallback / escalation) and tool config (noisy `request-start`, misleading `request-failed`, over-broad `function.description`, wrong handoff destination, self-referencing destination). Surface both as Phase 3 candidates next iteration. (3) Only after both above are exhausted → surface a clear stop with residual failures, hand off to the appropriate skill (`cekura-create-agent` for tool/config issues; backend team for upstream service errors).
...
- **Reading `llm_system_prompt` from the Cekura agent record in self-hosted mode, or asking the user to paste their prompt.** For `assistant_provider == "self_hosted"` (websocket), `llm_system_prompt` is almost always empty — the live prompt lives in the user's workspace as a source-file string constant. Do NOT pull `llm_system_prompt` and do NOT ask "paste your current system prompt so I can run improve-prompt against it." Instead, locate the prompt in the workspace: first the IDE-opened file (`ide_opened_file` context), then grep project files for the prompt string constant, and edit it directly via the `Edit` tool. Asking the user to paste is only acceptable in the explicit `offline` variant where no workspace is reachable.
...
└── providers/
    ...
    └── self-hosted/
        ├── overview.md                       # self-hosted umbrella, shared characteristics
...
- **[`providers/self-hosted/overview.md`](providers/self-hosted/overview.md)** — Self-hosted umbrella and shared characteristics; redeploy command flow.
