...
description: >
  ...
  agents (any agent the user runs themselves, defined by the run-setup in
  memory.md / CLAUDE.md, including the render-only no-live-target case).
license: MIT
...
**How it works.** The skill pairs autonomous iteration — diagnose → apply → re-validate with overfitting protection — with strict correctness gates: a faithful reproduction harness auto-built from the failing call / run, a **must-fail-first** reproduction gate before any edit, a **must-pass** verification gate with stochastic re-runs, a regression sweep, and a PR / summary step.

**Production-call inputs.** When the input is `call_ids` (or a `result_id` / `run_ids` pointing at production call logs), the skill runs end-to-end with **no manual mock/variable setup from the user**: it debugs the call, auto-populates mock tools + expected returns + dynamic variables from the call's own trace, reproduces the bug (gating on a definitive FAIL), iterates to a fix, verifies it (gating on a definitive PASS over 5–10 stochastic re-runs for probabilistic failures), runs a regression sweep, and either raises the PR or prints a PR-ready summary.
...
Currently supported for **VAPI**, **ElevenLabs**, and **self-hosted** (any agent the user runs themselves, operated per the run-setup). Retell support is intentionally disabled and will be re-enabled in a future revision.
...
```
   ...
   user input ─→  │  Setup phase   │  (phases/setup.md)
                  │  runs once     │
                  └───────┬────────┘
                          │  (mode, agent, redeploy_command, input_is_prod_call)
                          ▼
                  ┌────────────────────────────┐
                  ...
                  │  vars) → build evaluator (prefer     │    render-only skips entirely
...
- **Early-End-Call Diagnose** ([`phases/optimization/early-end-call-diagnose.md`](phases/optimization/early-end-call-diagnose.md)) — specialized triage for failures where the main agent ended the call before the scenario's required steps completed. Flags failures matching {main-agent-ended + scenario-incomplete in expected-outcome bullets} via a two-check verdict-first checklist (no rationale, no borderline cases), diagnoses root cause (too-permissive closure rules / orchestration-code end-of-call detection / VAPI handoff misconfigured), proposes minimal closure-rule prompt edits or — when the self-hosted run-setup edits source code — orchestration-code edits gating closure on captured state. Pass-through if no failures match. Proposed edits are NOT applied here; they flow into the combined proposal in Diagnose.
...
- **Sync** ([`phases/optimization/sync.md`](phases/optimization/sync.md)) — re-fetch the just-edited artifacts and verify each changed field landed correctly. Catches VAPI nested-object replacement, ElevenLabs wrong-path prompt no-ops, `Edit` ambiguous-anchor drift, and self-hosted stale-cache reads. Roll back to Apply on drift.
...
**Loop hand-off rules:**
- **Setup → Clone** (VAPI / ElevenLabs) when mode + agent are resolved. **Setup → Reproduce** directly for every `self_hosted` run (no clone), once mode + agent + `redeploy_command` + `input_is_prod_call` are all resolved.
...
- **Reproduce → Collect** when the must-fail-first gate is satisfied — a definitive FAIL in ≥ M of N runs, for both LLM-based and infra inputs. Reproduce hands forward the reproduction scenario IDs (now the loop input), the failure class, the full set, and the reproduction fail-run result URLs. If the bug does NOT reproduce (passes, or fails fewer than M of N), DO NOT enter the loop — surface the mock/variable-mismatch-or-stale-fix hypothesis and stop. The render-only case and simulation-run inputs that already have failing scenarios pass straight through once their (lighter) Reproduce steps complete.
...
- **`self_hosted`** — one bucket for any agent the user runs themselves. There are no sub-types and no routing question: the **run-setup** in `memory.md` / `CLAUDE.md` defines, generically, how the agent is **explored, edited, redeployed, and simulated**. The editable surface is whatever the run-setup points to — a source file (edited via `Edit`, covering system prompt, tool schemas, AND conversation-orchestration code), a row in a database (read via the user's SELECT, written via their UPDATE through the right CLI client), the Cekura record's mock-tool definitions (via `aiagents_partial_update`), or pasted text when there's no reachable live target (render-only — the skill prints the rewritten prompt and the user applies it). Business logic, security-sensitive code, LLM client config, and dependencies are out of scope regardless of surface. **The Cekura agent record's `description` / `llm_system_prompt` are NOT the source of truth — do not read them as the prompt.** `redeploy_command` makes edits live (a shell command, `"manual"`, `"noop"` when the runtime re-reads on every request, or render-only when there's no live target). See [`providers/self-hosted/overview.md`](providers/self-hosted/overview.md).
...
- Self-hosted: edits land via whatever the run-setup points to — the `Edit` tool on a source file (system prompt, tool schemas, and conversation-orchestration code all in scope), the user-provided UPDATE run through the right CLI client (`psql` / `mysql` / `sqlite3` / `sqlcmd` / `mongosh`) for a DB row (new prompt passed via env var or stdin, never a positional arg; credentials in-memory for the run only), `mcp__cekura__aiagents_partial_update` (`mock_tools` field) for Cekura mock-tool edits, or render-only when there's no reachable live target. The Cekura `description` is not the live prompt; sync it only if the user wants the dashboard to mirror the source. Full flow in [`providers/self-hosted/overview.md`](providers/self-hosted/overview.md).

## How to Use This Skill

This is an **interactive, multi-iteration workflow**. The user supplies one of:

- **VAPI / ElevenLabs / self-hosted modes (any live target)** — an `agent_id` plus exactly one of: `scenario_ids`, `result_id`, `run_ids`, or `call_ids`. When the input is `call_ids` (or a `result_id` / `run_ids` pointing at production call logs), the run is a production-call input: the Reproduce phase auto-builds the harness from the call's own trace (mock tools, expected returns, dynamic variables, testing-agent turns), gates on must-fail, the loop fixes, Eval gates on must-pass, Regression sweeps, and PR ships — no manual mock/variable setup.
- **Self-hosted, render-only (no reachable live target)** — a `prompt` (pasted text or read-only file path) plus pasted `{transcript, expected_outcome, verdict}` blocks. No live agent required.

Optionally:

- `dataset_size` (default 8, range 5–10) — number of *distinct* reproduction scenarios built for **LLM-based** failures (Reproduce REPRO.5). Infra failures ignore this (single evaluator) — but that single evaluator is still re-run `stochastic_runs` times.
- `stochastic_runs` (default 8, range 5–10) — how many times the skill auto-runs the evaluator for reproduction (REPRO.6) and verification (Eval EVAL.2). Applies to **both LLM-based and infra** failures (infra re-runs the single evaluator N times rather than once). The skill fires these itself; it does not ask the user to trigger each run.
...
- The user's input is ambiguous or incomplete (e.g., `agent_id` + `prompt` supplied without a mode; structured-config file where the prompt field can't be identified safely; empty / one-line / clearly-non-production prompt; the run-setup is silent on where a `self_hosted` agent's editable surface lives or how it's run).
...
- **Self-hosted source-file edits — edit the source the run-setup points to.** The run-setup in `memory.md` / `CLAUDE.md` is the authority on which file the live agent runs from; the IDE-opened file is a hint, not authority. If the run-setup doesn't name the source (or is ambiguous), ask the user which file is live rather than guessing. Files named `original_*.py`, `*.bak`, `*.snapshot.py`, anything under `archive/` or `backup/` are strong "probably not the live source" signals — pause and confirm rather than editing them.
- Self-hosted, render-only (no reachable live target) — there is no automated path to re-collect failures, so the skill must ask for pasted failures after each iteration.
...
1. **Setup** — load [`phases/setup.md`](phases/setup.md) and walk through Steps 1.1–1.4a. On completion, verify the Setup completion checklist before continuing. Record `input_is_prod_call`. For self-hosted live targets, Step 1.4 first checks `memory.md` / `CLAUDE.md` for saved run-setup instructions (redeploy command + how to launch and connect the main-agent simulation) and reuses them; Step 1.4a **persists** newly-collected setup back to `memory.md` so future runs don't re-ask.
1a. **Clone (VAPI / ElevenLabs only, runs once)** — load [`phases/clone.md`](phases/clone.md) and walk through Steps CLONE.1–4. Clone the provider agent + every referenced tool in the same provider org, duplicate the Cekura agent (`copy_scenarios=true`) in the same project, repoint it at the cloned provider id, and rebind the run to the clone. Skipped for every `self_hosted` run. A failed clone halts the run — never edit the original.
1b. **Reproduce (runs once)** — load [`phases/reproduce.md`](phases/reproduce.md) and walk through Steps REPRO.1–6. For prod-call inputs: debug + root-cause, classify LLM-vs-infra, auto-build the harness (mock tools + expected returns + dynamic variables + testing-agent vars), construct the evaluator (prefer `expected_outcome`), branch dataset-vs-single (harness shape only), and **gate on a definitive must-fail-first reproduction** (auto-run 5–10× and require ≥M/N fails for both LLM-based and infra). If the bug does not reproduce, surface and stop — do NOT enter the loop. Scenario / simulation-run inputs pass through the harness-building steps; the render-only (no-live-target) case skips this phase. Hands the reproduction scenario IDs + failure class + full set to Collect.
...
7. **Overfitting Gate (iteration N)** — load [`phases/overfitting-gate.md`](phases/overfitting-gate.md) and walk through Steps GATE.1–7. Inventories this iteration's edits, scores them against five overfitting signatures (verbatim transcript quote, scenario-specific identifier, hardcoded test-data value, hyper-narrow case clause, transcript-cloned few-shot example), decides REVISE / STRIP / KEEP per flagged edit, and applies cleanup edits if needed. On no-flag iterations the gate is a one-line pass-through to Eval (no extra apply round-trip). Code-stream edits (self-hosted orchestration code) and pure-deletion edits are not scored by the gate.
8. **Eval (iteration N)** — load [`phases/eval.md`](phases/eval.md) and walk through Steps EVAL.1–4. Validation runs apply the **must-pass stochastic gate** (≥M/N runs for both LLM-based and infra). Eval's Step EVAL.4 emits exactly one of these decisions:
...
- **Treating Cekura's `description` or `llm_system_prompt` as the system prompt in any self-hosted mode.** Neither is the source of truth — the prompt and how to apply it are owned by the user's run setup (the run-setup instructions in `memory.md` / `CLAUDE.md`, or what the user provides). Don't read or edit the Cekura record as the prompt.
- **Applying `Edit` with a non-unique `old_string` in self-hosted source-file edits.** The Edit tool fails on ambiguous matches. Use enough surrounding context (5–10 lines on either side) for every anchor.
- **Hallucinating variable-injection findings without runtime state.** Especially common in self-hosted runs where the runtime doesn't echo state back. Don't claim "the runtime didn't receive `{{accountId}}`" unless the transcript itself shows the placeholder leaking.
...
- **Quitting the loop the moment failures look non-prompt.** The exit gate is 100% pass rate or the iteration cap — not "first sight of an infra-shaped failure." Re-classify with fresh eyes before declaring upstream. When the self-hosted run-setup edits source code, also check whether the failure is a **CodeBug** (history truncation, missing forwarding, broken state) — those are in-scope for editing, not hand-offs.
...
- **Touching business logic, auth code, or dependencies in self-hosted code edits.** Orchestration-code edits are scoped to plumbing: history management, message wiring, state preservation, keepalive. Tool implementation bodies, API keys / auth code, secrets handling, dependency lists, and framework imports remain out of scope. When in doubt, hand off rather than edit.
- **Proposing code edits in VAPI, ElevenLabs, or render-only self-hosted runs.** The orchestration-code stream exists only when the self-hosted run-setup edits source code. In VAPI / ElevenLabs, code-shaped findings become upstream hand-offs — the skill cannot reach managed-provider infrastructure code; and a render-only run has no live file to edit.
...
- **Skipping the Optimization · Sync Step SYNC.1 re-fetch.** VAPI's PATCH semantics replace nested objects wholesale; a malformed body can silently wipe `messages` or `destinations` while returning 200. For self-hosted source-file edits, an `Edit` call with an ambiguous anchor can land in the wrong spot. Always re-read and verify.
- **Treating Cekura mock-tool edits as live-agent changes.** Cekura mock-tool definitions describe the testing contract; the agent's real implementations live in the user's code. Editing a mock-tool description without asking the user to update the live tool can produce edits that look fine on Cekura but never reach the agent.
...
- **Single-shot reproduction or verification — for either class.** Probabilistic agent behavior needs a sample, and so does real-transport infra: timing/audio/latency/interruption failures are intermittent even when the cause is deterministic code. Running the evaluator once (and declaring the bug reproduced or the fix verified on that one run) is the dominant source of "looked good in dev, regressed in prod" miscalls. The skill auto-runs 5–10× and gates on ≥ M of N (fail for reproduction, pass for verification) for **both LLM-based and infra** failures. No failure class gets a single run anymore — the LLM-vs-infra split drives only the harness shape (dataset vs. single evaluator), not the run count.
...
- **Building a dataset for an infra failure, or a single evaluator for an LLM failure.** The harness *shape* (not the run count) is driven by the LLM-vs-infra classification (REPRO.2): LLM-based → dataset of N *varied* scenarios (so a real fix is distinguishable from a lucky sample); infra → a single evaluator (the trigger is fixed, so varied scenarios add nothing). Note this is only about how many *distinct* scenarios you build — both classes still re-run their evaluator(s) 5–10× at the gate. Getting the shape backwards either fabricates needless variation for a fixed-trigger bug or under-samples the trigger space of a probabilistic one.
...
└── providers/
    ...
    └── self-hosted/
        └── overview.md                       # one generic bucket — explore/edit/redeploy/simulate per run-setup; edit-mechanism menu (file Edit / DB CLI / mock tools / render-only); redeploy flow
...
- **[`phases/optimization/apply.md`](phases/optimization/apply.md)** — Apply the combined edit set via the mode's machinery, then run `redeploy_command` (or fire manual restart gate). Non-zero exit halts.
...
- **[`phases/eval.md`](phases/eval.md)** — Validation-set construction (failure set vs. full set), validation execution with the **must-pass stochastic gate** (≥M/N runs for both LLM-based and infra), failure re-collection, decision tree (loop / in-loop sweep / converge → Regression / stop condition), iteration cap.
...
- **[`providers/self-hosted/overview.md`](providers/self-hosted/overview.md)** — The single generic self-hosted bucket: the four operations (explore / edit / redeploy / simulate) defined by the run-setup; the edit-mechanism menu (source file via `Edit` covering prompt + tools + orchestration code; database row via the user's SELECT/UPDATE through the right CLI client; Cekura mock tools via `aiagents_partial_update`; render-only when there's no live target); the redeploy-command flow (collection, sentinels, execution); apply/sync/no-change framing; edit-surface scope; runtime-state observability; edge cases; anti-patterns.
