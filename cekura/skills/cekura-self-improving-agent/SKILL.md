...
- Self-hosted / pipecat: mock-tool edits go through Cekura platform tools (`mcp__cekura__aiagents_partial_update` with `mock_tools` field for tool create/update/delete); prompt edits are applied per the user's run setup, not on the Cekura record. Full flow and redeploy gate in [`providers/self-hosted/pipecat.md`](providers/self-hosted/pipecat.md).
...
└── providers/
    ...
    └── self-hosted/
        ...
        ├── pipecat.md                        # pipecat sub-flavor — mock tools; prompt per user run setup
...
- **[`providers/self-hosted/pipecat.md`](providers/self-hosted/pipecat.md)** — Pipecat sub-flavor gate, Setup Step 1.3b summary, Cekura-side PATCH bodies for mock tools, redeploy gate, pipecat-specific edge cases. The prompt is owned by the user's run setup, not the Cekura record.
