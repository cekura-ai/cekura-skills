...
Execute the validation set in voice mode for VAPI. Capture `result_id`, poll until terminal (same 30s cadence and 15-min cap as Optimization · Collect Step COLLECT.1). For self-hosted / websocket / `file`, the same Cekura-driven execution applies — the validation runs hit the live agent the user just (hopefully) restarted.
