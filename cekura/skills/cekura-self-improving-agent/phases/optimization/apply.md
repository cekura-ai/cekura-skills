...
- `redeploy_command` is recorded on run state (set during Setup Step 1.4) — either a real shell command, the literal `"manual"`, or VAPI (which doesn't need one).
...
Skip entirely for VAPI and for the websocket `offline` variant. For websocket / `file`, branch on `redeploy_command`:
...
- **`redeploy_command == "manual"` (or unset and `auto_mode: false`)** → fire the websocket manual restart gate. Wait for explicit user confirmation (`done` / `restarted` / `redeployed` / `yes`).
...
After Step APPLY.2 (or after Step APPLY.1 for VAPI / offline), hand off to [`sync.md`](sync.md) with:
