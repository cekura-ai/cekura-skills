...
- Source-of-truth artifacts loaded? (VAPI: `/assistant/{id}` + tools; ElevenLabs: `/v1/convai/agents/{id}` + referenced `/v1/convai/tools/{id}`; self_hosted: the editable surface the run-setup points to — source file / DB row / Cekura mock tools / pasted text — ask the user if unspecified)
...
For self-hosted agents, scenario execution still runs against the live agent the run-setup points to. In auto mode the skill triggers validation without pausing to ask if the user has redeployed / restarted; if results look unchanged across iterations, the no-change hypothesis is surfaced after the fact (see Eval phase Step EVAL.4).

When there's no reachable live target (render-only), there are no scenarios to execute — only pasted failures.
...
| Pasted failures (render-only, no live target) | Trust the user's `{transcript, expected_outcome, verdict, verdict_explanation}` blocks. No fetch. Treat each block as a single failing run with no metric evaluations beyond what's pasted. |
...
**Self-hosted caveats.** Self-hosted runtimes generally do not expose `assistantOverrides.variableValues` or a fully-rendered system message the way VAPI does — the user's own agent controls what Cekura sees. The signals available are typically: (1) the test-profile / scenario variable values Cekura passed at run start (Signal 1, intent — visible on the run record), (3/4 partial) the transcript and any tool-call records Cekura captured (when the agent forwards `{"role": "Function Call", "data": {...}}` messages, Signal 4 is recoverable; many agents don't echo variable values back), and (5) `metadata.ended_reason` on the run record. Substitution failures usually surface as the agent literally speaking `{{variableName}}` or a tool call with placeholder arguments. Treat substitution as not-observable unless the transcript literally shows `{{varName}}`; with no live target (render-only), only Signals 3, 4-partial, and 5 (inferred from the transcript tail) are available. If a failure looks variable-injection-shaped but you cannot confirm runtime state, mark the diagnosis "suspected upstream — runtime state not observable" and surface the gap rather than blindly proposing a prompt edit in later sub-phases.
...
**Self-hosted runs skip this section entirely** — there's no managed-provider log surface to consult, and the user's own infrastructure logs are out of band for this skill. Telephony-shaped failures in self-hosted runs route directly to Upstream/data hand-off without an adjacent-log confirmation step.
