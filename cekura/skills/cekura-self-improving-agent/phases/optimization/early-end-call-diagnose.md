...
```
Run <id>:
  [ ] main agent ended the call?       (metadata.ended_reason == "main-agent-ended-call" for self-hosted + ElevenLabs;
                                        for ElevenLabs also YES if the transcript shows the agent firing the built-in
                                        end_call tool before required steps completed;
...
2. **Orchestration code's end-of-call detection** *(self-hosted, only when the run-setup edits source code)*. Open the source file and locate the end-of-call detection logic — typically a function or branch that scans agent / user messages for keywords ("goodbye", "bye", "thanks") and closes the connection. **Common shape**: the loop ends on any agent message containing a closure keyword, regardless of conversation state.
...
4. **Built-in `end_call` tool firing too eagerly** *(ElevenLabs only)*. If the closure rules are tight but the agent still fires `end_call` early, the built-in tool's own description / trigger may be too permissive. The fix is still primarily a prompt-side closure-rule edit (instruct the agent when NOT to call `end_call`); if the built-in tool config itself needs changing, surface it as a hand-off (built-in tools are config flags, not editable bodies — see [`../../providers/elevenlabs/overview.md`](../../providers/elevenlabs/overview.md)).

Pick the layer that, if fixed, would produce the largest behavior change. For VAPI, ElevenLabs, and self-hosted runs that don't edit source code, the prompt layer is almost always the right surface (orchestration code is out of reach there; tool-handoff edits apply only to VAPI and are rare causes; the ElevenLabs `end_call` built-in is a config-flag hand-off). When the self-hosted run-setup edits source code, prefer the prompt edit when the prompt's closure rules are clearly too permissive; reach for the orchestration-code edit when the prompt is fine and the code-level keyword scan is what fires.
...
- *IVR-loop early-end shape (DTMF-capable agents only)*: agent ended the call after the same IVR menu played repeatedly with no human-readable progress in the transcript (the DTMF tone fired but the turn was otherwise silent, or the agent leaked its conversational opener to the menu instead of pressing a digit). Root cause is usually that the prompt does NOT require a same-turn digit announcement, so the scenario evaluator can't detect progress and the loop reaches the bounded-skip-turn limit. Apply the canonical **DTMF / IVR navigation — same-turn digit-announcement pattern** from [`../../references/phase-3-diagnosis.md`](../../references/phase-3-diagnosis.md) § Recurring prompt-edit patterns. Pair with the existing "if you hear menu options, do X" section in the prompt, not a generic edge-cases block.

**Orchestration-code edit (self-hosted, source-file edits only, optional).** When the diagnosis is "code-level keyword scan ends the loop too eagerly", propose a minimal `Edit` adding a `required_fields_collected` check before the loop accepts a closing turn. Keep it scoped: one `Edit`, 5–10 lines of surrounding context per anchor. Do not refactor the loop; do not touch business logic. If the orchestration-code edit would require non-trivial refactor (new helper functions, multi-file changes), surface as a hand-off instead of attempting it.
