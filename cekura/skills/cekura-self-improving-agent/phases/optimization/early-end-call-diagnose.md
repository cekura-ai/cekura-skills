...
Pick the layer that, if fixed, would produce the largest behavior change. For VAPI / ElevenLabs / pipecat / websocket / offline modes, the prompt layer is almost always the right surface (orchestration code is out of reach for VAPI / ElevenLabs / pipecat / offline; tool-handoff edits apply only to VAPI and are rare causes; the ElevenLabs `end_call` built-in is a config-flag hand-off). For websocket / `file` mode, prefer the prompt edit when the prompt's closure rules are clearly too permissive; reach for the orchestration-code edit when the prompt is fine and the code-level keyword scan is what fires.
