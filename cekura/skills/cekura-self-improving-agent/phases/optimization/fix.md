...
- **Prompt edits** — change the system prompt on the managed provider's live configuration, or wherever the run-setup points (self-hosted — source file via `Edit`, DB row, or rendered for the user when there's no live target).
- **Tool-config edits** — change a tool's `name` / `description` / parameter (or `api_schema`) or its references using the provider's live configuration surface. Preserve provider-specific routing, modality, and built-in-tool semantics. Self-hosted edits tool-definition blocks in source (`Edit`, live on restart), a DB tools row, or Cekura mock tools (`description` / `mock_data` / `freetext_params` via `mcp__cekura__aiagents_partial_update` — GET → merge → PATCH the full `mock_tools` list; mock edits change only the testing contract). Built-in tools are config flags, not editable bodies — surface as hand-offs. Empty when there's no live target (render-only) → tool findings become hand-offs.
- **Code edits** *(self-hosted, only when the run-setup edits owned source code)* — when the root cause is in owned code rather than prompt wording (aggressive history truncation dropping earlier answers, tool-result not forwarded to the LLM, keepalive/retry losing turns, state slicing that breaks mid-conversation). Owned code includes a **vendored or forked SDK that lives inside the source tree the run-setup edits** — that is in scope (CodeBug), not Upstream. Scope is plumbing/orchestration only. **Out of scope always:** business logic (what a tool computes / an external service returns), auth / secrets / signing, dependencies, framework upgrades, LLM-client config. Managed providers and self-hosted runs that don't edit source never produce this stream — code-rooted findings there become upstream hand-offs. A code edit lands via `Edit` + redeploy and is re-validated on Cekura (the bug forced to reproduce in-sim per REPRO.3e).
- **Upstream hand-offs** — failures rooted in missing / wrong dynamic variables (no prompt or tool edit fixes them). Name the variable, what the prompt expected, what the runtime saw, and where to inject (test profile, scenario config, squad / project defaults, upstream caller). "Upstream" is reserved for code the user genuinely cannot edit; owned code is never Upstream. For self-hosted also consider the live code may not be reading the variable Cekura passes, and runtime state may be unobservable. In managed-provider and self-hosted-no-source runs, code-shaped findings surface here too.
...
- **The main agent ended the call** — `metadata.ended_reason` or the provider-native end reason shows a main-agent ending (for example, VAPI `assistant-ended-call` / `assistant-said-end-call-phrase`). Caller / timeout / silence / disconnect endings are **not** early-end — only main-agent-caused endings count.
...
- *Managed providers:* re-fetch the provider configuration + every referenced tool via the provider source-of-truth API/MCP.
...
- **Adjacent provider & telephony logs** *(managed providers, when COLLECT.4 pulled them)* — for tool-shaped failures quote the provider request/response trace, webhook status/body, and latency; for telephony-shaped failures quote the carrier status and error code. These often pin the diagnosis: a provider 4xx/5xx is a tool-config issue, not a prompt issue. If logs were not fetched because auth was unavailable, be conservative about Upstream/data.
...
- **Tool-config edits.** Four sub-types apply across modes: edit, add, remove a reference, or delete only after checking references. Managed providers use their live API/MCP; self-hosted uses the run-setup surface. Tool implementations, auth, and built-in tool bodies stay out of scope; if a mock edit matters only because the live signature differs, hand off ("update the live tool signature"). Built-in tools → hand-off.
