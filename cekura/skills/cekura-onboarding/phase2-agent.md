...
**Variants / forks / wrappers of a named provider → treat as custom, not the parent.** Many products are built ON one of the listed providers but are not that provider — e.g. Dograh (Pipecat-based, but `dgr_` keys dispatched via pipecat.daily.co), or any "powered by X" platform, self-hosted fork, or in-house wrapper. If the user names something that isn't verbatim on the list, or gives credentials whose shape doesn't match the parent's (different key prefix, different dashboard, an env var instead of a console key), **do NOT apply the parent provider's credential instructions** — those dashboard paths and key locations will be wrong and send the user hunting in the wrong place. Instead: keep whichever `provider.type` actually dispatches their calls (ask them if unsure — for Dograh it's `pipecat`), but collect credentials the **custom/self-hosted way**: ask generically ("what's the API key / connection detail for your setup, and where do you get it?") without asserting a specific menu path you can't verify. When in doubt, telephony (a phone number) is the most portable connection and needs no provider credentials at all.
