...
| **LiveKit · Pipecat** | **2b** | hand off to `cekura-livekit-pipecat-onboarding` — **never 2c** |
| self-hosted / custom, or a fork built on a named provider | **2c** | manual |

**2c is NOT a LiveKit/Pipecat destination.** Their entire flow — including the paste path when GitHub is declined — lives in the dedicated skill. Landing in 2c for LiveKit/Pipecat skips the GitHub check and the repo scan, and the first visible symptom is asking the user for a name and a system prompt that were sitting in their code.
 **Onboarding is self-contained — do NOT open the cekura-create-agent skill or any of its phase files during onboarding** (its phase sequence covers post-onboarding work like mock tools and knowledge bases; running it mid-onboarding hijacks the flow). SDK integration for LiveKit/Pipecat lives in `cekura-livekit-pipecat-onboarding`. The credential matrix you need:
...
- **LiveKit and Pipecat are the exception to that rule and to every "collect the credentials first" instruction in this file.** They never defer and they never collect: the dedicated skill creates the agent with marked placeholder credentials and has the user replace them on the agent page. Hand off in 2b before applying anything above to them.
...
## 2b. LiveKit / Pipecat — hand off to the dedicated skill

Nothing below this heading is run from this file. The code-based flow — GitHub check → repo scan → placeholder-credential create → first evaluators and run → SDK pull request — lives in the **`cekura-livekit-pipecat-onboarding`** skill. **Hand off NOW, before any other question:**

- Claude Code: `Skill(skill="cekura:cekura-livekit-pipecat-onboarding")`
- Any other harness: read `cekura/skills/cekura-livekit-pipecat-onboarding/SKILL.md` and follow it phase by phase.

Pass this block verbatim as the first thing it sees:
...
- Agent: none yet | <agent_id> (<name>) — resume
- User already said: <repo name / dispatch name / language / anything volunteered, or "nothing">
- Session: dashboard (dashboard_url=<from the workspace line>) | local (repo at <path>)
```

**When it completes, onboarding is complete** — do not run 3T–6T again; its closing summary uses the shape in [phase6-testing-next.md](phase6-testing-next.md). It handles every decline internally (its own paste path when GitHub is declined) and exits only to that summary. **A fork or wrapper built on LiveKit/Pipecat never enters it** — that is `custom`, in 2c, per the carve-out above.

## 2c. Manual essentials (self-hosted / deferred-key providers)

> **Entry check — LiveKit and Pipecat never arrive here.** Their paste path (for a declined GitHub) is inside `cekura-livekit-pipecat-onboarding`. If the provider is LiveKit or Pipecat, stop and hand off per [2b](#2b-livekit--pipecat--hand-off-to-the-dedicated-skill) — the name and the system prompt are in their repo, and asking for them is the symptom of having skipped it.
...
- Agent name, language.
- Connection details for how Cekura reaches the agent, in order of preference: existing phone number → SIP URI → websocket URL → provider WebRTC.
...
**LiveKit and Pipecat: this gate and everything after it are owned by `cekura-livekit-pipecat-onboarding`.** When that skill completes, onboarding is complete.
