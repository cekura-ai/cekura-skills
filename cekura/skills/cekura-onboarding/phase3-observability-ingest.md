...
**Default for VAPI / Retell / ElevenLabs / Synthflow (when the provider API key is on the agent): auto-fetch production calls.** Enable it with `aiagents_auto_fetch_create` (poll `aiagents_auto_fetch_progress_retrieve`) — Cekura pulls the calls from the provider directly, no webhook or manual upload needed. Bland supports agent configuration import and prompt sync, but not periodic production-call import. Only fall back to the modes below if the provider is unsupported or its API key was deferred in Phase 2 (surface that as the reason and offer to add the key now).
