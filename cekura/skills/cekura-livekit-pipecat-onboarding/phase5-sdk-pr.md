...
**Testing variant: do not open this phase, mention the SDK, mention tracing, or mention a pull request until one call in THIS conversation has produced a two-sided transcript.** The rail is: GitHub → scan → create the agent → user confirms credentials → generate evaluators → ASK → run → the user's agent answers → *then* this file. If any of those has not happened, you are in the wrong phase — go back to [phase3-create.md](phase3-create.md) or [phase4-evaluators-run.md](phase4-evaluators-run.md) and finish it.

**One two-sided transcript is the entire bar**, and these are explicitly NOT part of it:

- **A passing evaluation.** Arriving here from a run where four of ten failed is normal and expected — [phase4-evaluators-run.md](phase4-evaluators-run.md) 4e routes here directly from that case. Report the failures, link the results page, and carry on. Never treat a red number as a reason to withhold this step.
- **Finished metric scores.** Evaluation trails the call by a minute or two, and that wait is exactly when this step belongs. If you arrived from phase4 with the run still scoring, keep the result id: after the PR is raised or declined, read the results once more and share the scores before the closing summary.
- **The rest of the scenarios.** "I'll run the rest later" is a normal way to arrive here.

What you may NOT be here without is a call the user's agent actually answered and spoke in. Not an agent id, not a scenario id, not a plan to run later, and not a run where every call failed to connect — that one is phase4's F5 and it stays a credentials question. The offer is "here's what more Cekura can capture", a sentence that means nothing to someone who has not yet watched their agent take a call, and asking them to review a change to their own repo before that is asking for trust they have no reason to give. The runtime enforces the coarse tail of this: `github_open_pull_request` is denied until a `scenarios_run_*` has come back clean. **Observability variant:** you arrive here straight from [phase3-create.md](phase3-create.md) — the SDK *is* the observability integration for these providers — with the use case preselected to "Observability"; skip the pitch's testing half.
...
- **"Not now"** — fallback **F6**: accept it in one line, leave `tracing_enabled` at `false`, and exit to the closing summary in [phase6-close.md](phase6-close.md). Do not re-pitch.
...
Close out with the summary from [phase6-close.md](phase6-close.md), including the PR link and any open item. **That summary is the end of onboarding** — onboarding's own phases are not re-run.
