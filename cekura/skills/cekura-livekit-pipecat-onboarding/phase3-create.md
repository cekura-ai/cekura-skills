...
**"Not yet"** — this is fallback **F3** in SKILL.md. Ask what they want: `["Done — they're updated now", "Generate evaluators anyway — I'll add the keys before the run", "Pause here"]`. *Generate anyway* → [phase4-evaluators-run.md](phase4-evaluators-run.md), and re-ask about the credentials before the run. *Pause* → exit to the closing summary (`cekura-onboarding/phase6-testing-next.md`) with "placeholder credentials" as the open item.
