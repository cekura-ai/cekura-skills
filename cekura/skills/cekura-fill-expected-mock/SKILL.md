---
name: cekura-fill-expected-mock
description: >
  Use when a Cekura evaluator, especially a manually authored conditional-actions / structured /
  scripted scenario, needs its expected mock tool calls (`generated_mock_tool_entries`) and matching
  Cekura mock-tool data created, filled in, backfilled, repaired, or validated. Trigger when users ask to
  fill in expected mock tool calls, add expected mock calls, backfill tool input/output for manual scenarios,
  fix missing mock data after scenario creation, align test profiles with mock outputs, or prevent
  manually authored Cekura scenarios from being blindsided by absent expected mock tool calls. This is
...
# Cekura Fill Expected Mock Tool Calls

## Purpose

Populate a scenario's **expected mock tool calls** (the `generated_mock_tool_entries` field) when it has none, and make sure they line up with the agent's mock tools and test profile.
...
**Load `cekura-eval-design` first and design the data there.** This skill picks up once that data is designed and covers only the expected-mock-tool-call work: inferring which tool calls a manual scenario should trigger, assembling and attaching `generated_mock_tool_entries`, and verifying it matches the agent's mock data.
...
1. **Load companion guidance.** Read `cekura-eval-design` (especially `references/test-data-design.md`) for the scenario type and the mock-data / test-profile design rules, then read `references/workflow.md` in this skill for the fill-in procedure.
...
5. **Assemble the expected mock tool calls.** Build one `generated_mock_tool_entries` entry per exercised tool, each carrying the exact input/output that will be present in the agent's mock data.
...
7. **Attach the entries via REST.** Set `generated_mock_tool_entries` with `PATCH /test_framework/v1/scenarios/{id}/`. This field is not part of the scenario create/update request schema, so it can only be set through the REST scenario endpoint — see `references/workflow.md`.
8. **Verify.** Retrieve the agent, test profile, and scenario again. Confirm the scenario's expected mock tool calls exactly match the entries present in the agent's mock data and the values referenced by the test profile.
...
- **`references/workflow.md`** — data model, write paths for the expected mock tool calls and agent mock data, validation checklist, and failure handling.
