...
> **LiveKit / Pipecat note:** keep `provider.type = livekit` or `pipecat` regardless of how Cekura connects (phone, WebRTC, chat). Never reroute a LiveKit/Pipecat agent into `custom` just because it has a phone number — these providers support phone-based simulations and SDK integration natively under their own type. They are also the two **code-based** providers: nothing auto-imports, so Phase 2 hands them to `cekura-livekit-pipecat-onboarding`, which reads the configuration out of the agent's own repo and creates the agent with placeholder credentials rather than asking for a secret in chat; resume at Phase 7 when it returns.
>
