...
| DTMF (`DTMFAggregator`, `sendDtmfEnabled`) | **DTMF Multi-digit Processing** | `<dtmf digits="XXXXX#" />` + spoken confirmation |
...
| Internal processor state | Evaluators see only the transcript — not whether `LLMRetryProcessor` ran, not error counts, not internal flags |
