...
| Q5: idle detection exists — bot prompts on silence | **Mid-Call Idle** | `<hold>` for threshold + 2s |
...
| Scenario | Why |
|---|---|
| Call-start silence timeout | `<hold>` and `<silence>` both produce brief audio artifacts at their boundaries that can trigger voice-activity detection, resetting the timer before it fires |
