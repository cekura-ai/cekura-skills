...
Read `.cekura-infra/workflow-descriptions.md` (written by Phase 2) before doing anything else. That file is the authoritative source — every test item in this phase must be grounded in what Phase 2 actually documented. Do not add test items for behaviors that Phase 2 did not find.
...
Write the list to `.cekura-infra/test-list.md`. Phase 4 reads this file and groups items into evaluators using arc patterns.
...
Before writing any TEST-NNN items, open `.cekura-infra/workflow-descriptions.md` and count:
...
After writing items for every component section, re-read `.cekura-infra/workflow-descriptions.md` from start to finish. For every documented value, parameter, behavior, or code path that does not yet have a corresponding TEST-NNN item, add one. Do not stop until no uncovered items remain.
...
Write the complete test list to `.cekura-infra/test-list.md` using this structure:

```markdown
# Infra Test List

Generated by Phase 3 of cekura-infra-test-suite.
Source: .cekura-infra/workflow-descriptions.md
...
Then actually go back and expand — add the missing items to `.cekura-infra/test-list.md`, recount, and re-run this gate check.
