...
- **Leading `<silence>` on `id: 0` to wait out the greeting.** The agent's greeting cuts the silence, and the unplayed opener is dropped, not retried. Use `id: 0` `action: ""` plus a `standard` `id: 1` for the line.
