...
- **A step scheduled after the call ends.** An action containing `<endcall />` hangs up, so every later step, and every outcome line resting on one, is unreachable. Two possible endings are two evaluators, not two branches in one.
...
- [ ] Nothing is scheduled after an action containing `<endcall />`, and no `action_followup` references a condition that ends the call
