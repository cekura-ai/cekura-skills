...
**Which actions are fully uninterruptible.** Only actions whose entire payload is an `<ivr text="..." />` or `<voicemail text="..." />` tag are fully uninterruptible — both tags must occupy the whole action by validation rule, so there is no surrounding text for the main agent to interrupt. `<hold>` is only uninterruptible **during the hold period itself**; any spoken text before or after `<hold>` in the same action remains interruptible (so `"sentence <hold time=\"3s\" /> sentence"` is not safe inside an `action_followup`). `<interruption>` controls *when* the testing agent cuts in — it does not protect the spoken text that follows from being re-interrupted by the main agent.

**Practical rule:** if the action contains any interruptible content (plain spoken text, `<silence>`, or text adjacent to `<hold>`) AND the main agent might speak during it, use `type: "standard"` so the testing agent can adapt on interruption. Reserve `action_followup` for cases where the next testing-agent turn must fire regardless of what the main agent said (multi-part responses, scripted sequences, `<interruption>` placement) AND the action either completes too quickly to be interrupted in practice or is one of the fully uninterruptible `<ivr>` / `<voicemail>` forms. See "Anti-Patterns" and "Troubleshooting" for the failure signature.
...
- **`action_followup` wrapping interruptible content (plain spoken text, `<silence>`, or text adjacent to `<hold>`).** When the main agent interrupts, the testing agent re-runs the whole `action_followup` action from the start because there is no condition to re-match. If interruption recurs, the testing agent loops on the same sentence. Use `type: "standard"` for any action where the main agent might interrupt — on interruption, the testing agent re-evaluates conditions against the new context and adapts. Only actions whose entire payload is `<ivr text="..." />` or `<voicemail text="..." />` are fully uninterruptible; `<hold>` protects only the hold period itself, so surrounding text in the same action is still vulnerable.
...
- [ ] No `action_followup` action contains interruptible content — plain spoken text, `<silence>`, or text adjacent to `<hold>` (the hold period itself is uninterruptible, but surrounding text is not). Only entire-action `<ivr>` / `<voicemail>` payloads are fully uninterruptible. If interruption is possible, use `type: "standard"` so the testing agent re-evaluates conditions on interruption instead of looping on the same action.
...
| Testing agent keeps repeating the same sentence / opening line in a loop | An `action_followup` action contains interruptible content (plain spoken text, `<silence>`, or text adjacent to `<hold>`) and the main agent interrupts each cycle. On interruption, the testing agent re-executes the whole `action_followup` from the start because there is no condition to re-match. | Change the condition to `type: "standard"` so the testing agent re-evaluates conditions on interruption and adapts to the new context. Reserve `action_followup` for actions whose entire payload is an uninterruptible `<ivr text="..." />` or `<voicemail text="..." />` tag, or that the main agent is unlikely to interrupt. |
...
Action types:
  standard         Fires when conversation context matches condition string.
                   ...
                   → Use for any action where the main agent might interrupt (e.g., contains <silence>).
  action_followup  Fires on the NEXT TURN after condition id (int): testing agent sends
                   ...
                   to re-match). If interruption recurs, the testing agent LOOPS on the same
                   sentence. Only fully safe when the action's entire payload is <ivr text="..." />
                   or <voicemail text="..." />. <hold> protects only the hold period itself —
                   surrounding text in the same action is still interruptible.
