...
4. **Language-specific personalities may not be enabled per-project** — Non-English personalities (e.g., ID 4566 for Russian) may return "Personality is not enabled" errors. Always try the language-matched personality first (or a multilingual `language=multi` one when the scenario mixes languages); on that error, first try enabling or creating/forking a personality in the target language (`scenario_language` is coupled to the personality's language by design — mismatches are rejected); only if that isn't possible, fall back to a normal English personality (`scenario_language` stays `en`), keep the content in the target language via instructions, and disclose the TTS limitation.
