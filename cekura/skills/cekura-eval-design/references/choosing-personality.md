...
- Language-matched personality returns "Personality is not enabled" (or none exists) → fall back to a normal English personality (`personalities_list language=en`) and set `scenario_language` to the target language **on the create call itself** — `scenarios_create` accepts `scenario_language`, but the update endpoint does not expose it, so it cannot be patched afterwards
