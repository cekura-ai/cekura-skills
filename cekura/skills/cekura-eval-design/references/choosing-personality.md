...
- Language-matched personality returns "Personality is not enabled" (or none exists) → fall back to a normal English personality (`personalities_list language=en`). Try to pass `scenario_language: <target code>` on the create call; be aware the MCP create/update tools may not expose the field (`scenario_language` then follows the personality's language). If it doesn't stick, say so plainly in your summary and tell the user to set the scenario language in the dashboard — never claim it was set when it wasn't
