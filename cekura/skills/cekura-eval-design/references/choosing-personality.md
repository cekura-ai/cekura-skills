...
- Language-matched personality returns "Personality is not enabled" (or none exists) → fall back to a normal English personality (`personalities_list language=en`) using a **two-step**: create with the fallback personality and **without** `scenario_language` (the API rejects a create where `scenario_language` differs from the personality's language), then PATCH `scenario_language: <target code>` **by itself** on the created scenario. If the update tool does not expose `scenario_language` (older deployments), say so plainly in your summary and tell the user to set it in the dashboard — never claim it was set when it wasn't
