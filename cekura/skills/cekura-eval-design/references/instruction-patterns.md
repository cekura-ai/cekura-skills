...
- **Using instructions for voice characteristics** — Instructions like "speak in a mumbling voice", "be interruptive" or "use an Indian accent" don't change the testing agent's vocal style. Use **personalities** for that — they carry the real voice-layer settings (interruption level, background noise, speed, volume, idle timing, and the `voice_id` that determines the accent). Accent is not a field: it comes from the voice, so an accent request means picking a different `voice_id`.
