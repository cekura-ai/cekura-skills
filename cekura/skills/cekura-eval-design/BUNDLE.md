...
10. The flow ends once: `<endcall />` on the **last** action, or a terminal transfer, or the user asked to stay on the line. Nothing is scheduled after the ending — the call is over, so a later condition and any outcome line resting on it can never be reached. Two possible endings are two evaluators.
