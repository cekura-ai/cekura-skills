...
4. **Language-specific personalities may not be enabled per-project** — Non-English personalities may return "Personality is not enabled" errors. Always try the language-matched personality first (via `personalities_list` with `language=<code>` — the only filter needed; if a `project_id`-filtered lookup comes back empty, retry without `project_id` before concluding nothing matches — or a multilingual `language=multi` personality when the scenario mixes languages); on that error, remember `scenario_language` is **coupled to the personality's language by design** (mismatched creates are rejected — don't work around it): enable the predefined one for the project, or create/fork a Normal personality in the target language (`personalities_create` — multilingual voice models handle any supported language) and use it. Only if that write actually fails (permissions, plan limits) or the user declines, fall back to a normal English personality with `scenario_language` staying `en`, keep the caller-facing content in the target language, and disclose the TTS/transcription limitation plainly. See "Checking Available Personalities" under the Personality section.
