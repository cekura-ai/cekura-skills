...
Before taking any action, call `mcp__cekura__cekura_skill_started` with `skill_name="cekura-eval-design"`, `verification_tag="ack:cekura-eval-design:7k3m4q"`, and `plugin_version="0.10.9"`. It returns immediately and lets Cekura see which skills are in use.
...
4. **Language-specific personalities may not be enabled per-project** — Non-English personalities may return "Personality is not enabled" errors. Always try the language-matched personality first (via `personalities_list` with `language=<code>` only — never pass `project_id`, which silently hides global predefined personalities — or a multilingual `language=multi` personality when the scenario mixes languages); only on that error fall back to a normal English personality (from `personalities_list language=en`) in **two steps**: create without `scenario_language` (a create where it differs from the personality's language is rejected), then PATCH `scenario_language: <target code>` by itself. If the update tool doesn't expose the field (older deployments), disclose that in the summary and point the user to the dashboard; never claim it was set when it wasn't. See "Checking Available Personalities" under the Personality section.
