...
Generation attaches the project's simulation-enabled set — the project's own choice, so leave it in place; attach a baseline metric only when it is missing and the project has it. Every attached metric is scored on every run, and a project rubric rule over it gates the run's `success`, so when a scenario clearly cannot exercise an inherited flow-specific metric (a booking-flow metric on a pure FAQ scenario), say so in the summary and let the user decide; a trigger that returns N/A on that metric (see **cekura-metric-design**) is usually better than removing it from the scenario.
