...
4. **Language-specific personalities may not be enabled per-project** — Non-English personalities may return "Personality is not enabled" errors. Always try the language-matched personality first (via `personalities_list` with `language=<code>` only — never pass `project_id`, which silently hides global predefined personalities — or a multilingual `language=multi` personality when the scenario mixes languages); only on that error fall back to a normal English personality (from `personalities_list language=en`) and try to pass `scenario_language: <target code>` on the create call. The MCP create/update tools may not expose that field (it then follows the personality's language) — if it doesn't stick, disclose that in the summary and point the user to the dashboard; never claim it was set when it wasn't. See "Checking Available Personalities" under the Personality section.
