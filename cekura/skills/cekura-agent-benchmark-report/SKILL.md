...
6. run a weighted 25-, 50-, 75-, or 100-call batch;
...
Before creating scenarios, metrics, profiles, or runs, provide one concise checkpoint containing the selected agent, proposed evaluator count, representative workflows, attached metrics, and a choice of **25, 50, 75, or 100 batch calls**. State the coverage tradeoff: 25 is a focused smoke benchmark, 50 is a balanced directional read, 75 increases confidence across workflows, and 100 is the fullest repeatability signal. If the user already specified a different total, use that total instead. Show a cost estimate for the chosen option: projected pilot calls, the selected batch-call total, any reasonable evaluator-repair reruns, the applicable per-call and paid-metric charges (or the dashboard-provided total when available), and a conservative estimated-total range. Wait for explicit approval of both the selected batch size and cost before starting any paid pilot or batch call, even when the user has generally authorized evaluation. If pricing cannot be retrieved, state the run count and that the cost is unavailable, then wait for approval rather than guessing.
...
Do not change the agent to make a pilot pass. Do not start the selected batch while unresolved evaluator defects could invalidate the result. State clearly when a failure remains ambiguous and needs user direction.
...
Allocate exactly the approved batch total across the validated evaluators. The distribution may be uneven: assign more runs to critical workflows, high-risk branches, or paths with ambiguous/recent pilot behavior; retain at least one run for every validated evaluator. Record the selected total, allocation, and rationale in the report evidence ledger.
...
Treat the following as an atomic delivery gate after the selected batch completes:
