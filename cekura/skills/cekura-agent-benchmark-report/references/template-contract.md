...
`e4a0fc40531b0f7ed752e799ac607914679c7bc2a6c22b683a61a35386b7ed2f`
...
All four comparison cards always remain present. Each has every live Bench provider plus the selected agent; an unavailable Bench metric remains a visible row. The selected agent always receives its calculated task-completion position in the summary and leaderboard. Task completion retains the directional-suite note and a `*` rank marker whenever its suite differs from Bench. The latency section retains its four-value summary, overall bars, scenario chart, and ordinal response-turn chart. The scenario and response-turn charts are separate, simultaneously visible figures—never alternate views in tabs or a toggle. Where source data is missing, retain the card and say unavailable.
