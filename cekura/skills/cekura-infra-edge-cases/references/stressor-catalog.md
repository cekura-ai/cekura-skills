...
**Before creating personalities:** `personalities_list` filtered by the agent's language and inspect one existing personality to copy the exact nested JSON shape of `network_simulation`, `background_sound_volume`, `stop_speaking_plan`, and `message_plan`. The field names below are stable; the nested value shapes are provider-specific, so copy a real one rather than inventing it.
