...
|---|---|
| `conditional_actions` — drop, tool_error, workflow_miss (turn-by-turn replays) | `mcp__cekura__scenarios_create` with the drafted `conditions` when the replay is tag-driven or exact-script. For a grounded workflow-shaped replay, `scenarios_generate_bg` with `simulation_type: "conditional_actions"` emits validated conditions from the agent description. |
