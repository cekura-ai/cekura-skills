...
knowledge base and mock tools. Conditional actions are now generated by default
with `simulation_type: "conditional_actions"` — the generator knows the full tag set
and honours tag requirements in `extra_instructions` — and direct creation is
reserved for a user-dictated exact script or an exact timing value.
